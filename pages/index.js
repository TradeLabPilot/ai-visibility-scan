import { useState, useRef, useEffect } from "react";
import { Radar, ArrowRight, RotateCcw, TriangleAlert } from "lucide-react";

const BONUS_TEXT =
  "No cost, no obligation — we'll show you what we found either way.";

const SCAN_LINES = [
  "Asking the question your customers ask...",
  "Reading what actually ranks right now...",
  "Seeing who gets recommended...",
  "Looking up your business by name...",
  "Comparing you against the names it found...",
  "Writing up what is missing...",
];

export default function Home() {
  const [phase, setPhase] = useState("form");
  const [form, setForm] = useState({ businessName: "", industry: "", city: "", email: "" });
  const [lineIdx, setLineIdx] = useState(0);
  const [result, setResult] = useState(null);
  const [errorMsg, setErrorMsg] = useState("");
  const [leadEmail, setLeadEmail] = useState("");
  const [leadPhase, setLeadPhase] = useState("idle"); // idle | sending | done | error
  const [leadErr, setLeadErr] = useState("");
  const intervalRef = useRef(null);

  useEffect(() => {
    if (phase === "scanning") {
      intervalRef.current = setInterval(() => {
        setLineIdx((i) => (i + 1) % SCAN_LINES.length);
      }, 1100);
    }
    return () => clearInterval(intervalRef.current);
  }, [phase]);

  const handleChange = (field) => (e) =>
    setForm((f) => ({ ...f, [field]: e.target.value }));

  const runScan = async (e) => {
    e.preventDefault();
    if (!form.businessName.trim() || !form.industry.trim() || !form.city.trim()) return;

    setPhase("scanning");
    setLineIdx(0);
    const startedAt = Date.now();

    try {
      const response = await fetch("/api/scan", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(form),
      });

      const data = await response.json();

      // A rejected email (or any 4xx) comes back with a plain reason. Show it
      // right away instead of sitting through the scan animation first.
      if (!response.ok && data.error) {
        setErrorMsg(data.error);
        setPhase("error");
        return;
      }
      const elapsed = Date.now() - startedAt;
      const minWait = 0;
      if (elapsed < minWait) await new Promise((r) => setTimeout(r, minWait - elapsed));

      if (data.parsed) {
        setResult(data.parsed);
        setPhase("result");
      } else {
        setErrorMsg(
          "The check came back incomplete this time. Tap Try again - it usually works on the second run."
        );
        setPhase("error");
      }
    } catch (err) {
      setErrorMsg("Scan failed — couldn't reach the live check. " + err.message);
      setPhase("error");
    }
  };

  const submitEmail = async (ev) => {
    ev.preventDefault();
    const value = leadEmail.trim();
    if (!value) return;
    setLeadPhase("sending");
    setLeadErr("");
    try {
      const res = await fetch("/api/subscribe", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          email: value,
          businessName: form.businessName,
          industry: form.industry,
          city: form.city,
          cited: result ? result.cited : null,
          competitors: result && result.competitors ? result.competitors : [],
        }),
      });
      const data = await res.json();
      if (!res.ok) {
        setLeadErr(data.error || "Could not save that address.");
        setLeadPhase("error");
        return;
      }
      setLeadPhase("done");
    } catch (err) {
      setLeadErr("Could not reach the server. Try again?");
      setLeadPhase("error");
    }
  };

  const reset = () => {
    setPhase("form");
    setResult(null);
    setErrorMsg("");
    setLeadEmail("");
    setLeadPhase("idle");
    setLeadErr("");
  };

  const card = { background: "#FFFFFF", border: "1px solid #D3DEE7", borderRadius: 16, padding: 24 };
  const label = { color: "#5A7186", fontSize: 12, fontFamily: "ui-monospace, monospace", textTransform: "uppercase", display: "block", marginBottom: 4 };
  const input = { background: "#F4F7FA", border: "1px solid #D3DEE7", color: "#16232E", width: "100%", borderRadius: 8, padding: "8px 12px", outline: "none", boxSizing: "border-box", fontSize: 14 };
  const goldBtn = { background: "#0084CC", color: "#FFFFFF", width: "100%", borderRadius: 8, padding: "12px 0", fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, border: "none", cursor: "pointer", fontSize: 15, textDecoration: "none" };

  return (
    <div style={{ background: "#F4F7FA", minHeight: "100vh", color: "#16232E", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "system-ui, sans-serif" }}>
      <style>{`
        @keyframes spinSweep { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
        @keyframes pulseGlow { 0%, 100% { opacity: 0.45; transform: scale(0.92); } 50% { opacity: 1; transform: scale(1.12); } }
        @keyframes blipIn { from { opacity: 0; transform: scale(0.4); } to { opacity: 1; transform: scale(1); } }
        .sweep-anim { animation: spinSweep 2.8s linear infinite; }
        .pulse-glow { animation: pulseGlow 1.6s ease-in-out infinite; }
        .blip-in { animation: blipIn 0.5s ease-out both; }
      `}</style>

      <div style={{ width: "100%", maxWidth: 448 }}>
        <div style={{ marginBottom: 32, textAlign: "center" }}>
          <div style={{ color: "#6E8598", letterSpacing: "0.18em", fontSize: 12, fontFamily: "ui-monospace, monospace", textTransform: "uppercase", marginBottom: 12 }}>
            Agency AI Lab — Visibility Scan
          </div>
          <h1 style={{ fontFamily: "system-ui, -apple-system, 'Segoe UI', sans-serif", letterSpacing: "-0.01em", color: "#16232E", fontSize: 30, lineHeight: 1.2, marginBottom: 8 }}>
            AI has already picked<br />who to recommend.
          </h1>
          <p style={{ color: "#5A7186", fontSize: 14 }}>
            Find out who it named in your category — and whether you're on the list.
          </p>
        </div>

        {phase === "form" && (
          <form onSubmit={runScan} style={{ ...card, display: "flex", flexDirection: "column", gap: 16 }}>
            {[["businessName", "Business name"], ["industry", "Industry / trade"], ["city", "City"]].map(([field, text]) => (
              <div key={field}>
                <label style={label}>{text}</label>
                <input value={form[field]} onChange={handleChange(field)} style={input} placeholder={field === "city" ? "e.g. Orlando, FL" : ""} />
              </div>
            ))}
            <button type="submit" style={{ ...goldBtn, marginTop: 8 }}>
              <Radar size={18} /> Run Visibility Scan
            </button>
            <p style={{ color: "#5A7186", fontSize: 12, textAlign: "center", marginTop: 10, lineHeight: 1.45 }}>
              Takes up to a minute. It runs live searches, not a lookup.
            </p>
          </form>
        )}

        {phase === "scanning" && (
          <div style={{ ...card, display: "flex", flexDirection: "column", alignItems: "center", padding: 32 }}>
            <div style={{ position: "relative", width: 180, height: 180 }}>
              <div style={{ position: "absolute", inset: 0, border: "1px solid #D3DEE7", borderRadius: 9999 }} />
              <div style={{ position: "absolute", inset: 22, border: "1px solid #D3DEE7", borderRadius: 9999 }} />
              <div className="sweep-anim" style={{ position: "absolute", inset: 0, borderRadius: 9999, background: "conic-gradient(from 0deg, rgba(0,132,204,0.35), transparent 35%)" }} />
              <div className="pulse-glow" style={{ position: "absolute", width: 14, height: 14, borderRadius: 9999, background: "#0084CC", top: "50%", left: "50%", marginTop: -7, marginLeft: -7 }} />
              {[0, 1, 2].map((i) =>
                i <= lineIdx ? (
                  <div key={i} className="blip-in" style={{ position: "absolute", width: 8, height: 8, borderRadius: 9999, background: "#C0392B", top: `${20 + i * 25}%`, left: i % 2 === 0 ? "18%" : "76%" }} />
                ) : null
              )}
            </div>
            <div style={{ color: "#5A7186", fontFamily: "ui-monospace, monospace", fontSize: 14, marginTop: 24, textAlign: "center" }}>
              {SCAN_LINES[lineIdx]}
            </div>
            <div style={{ color: "#16232E", fontSize: 15, marginTop: 14, textAlign: "center", maxWidth: 300, lineHeight: 1.45 }}>
              &ldquo;Who is the best {form.industry || "provider"} in {form.city || "your area"}?&rdquo;
            </div>
          </div>
        )}

        {phase === "result" && result && (
          <div style={card}>
            <div style={{ color: result.cited ? "#0084CC" : "#C0392B", fontSize: 24, fontWeight: 700, marginBottom: 8 }}>
              {result.cited ? "AI named you" : "AI did not name you"}
            </div>
            {result.confidence && (
              <div style={{ color: "#5A7186", fontSize: 12, fontFamily: "ui-monospace, monospace", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 12 }}>
                Confidence: {result.confidence}
                {result.confidence === "low" ? " — thin results, treat as directional" : ""}
              </div>
            )}
            <p style={{ color: "#16232E", fontSize: 14, marginBottom: 16 }}>{result.note}</p>

            {result.gaps && result.gaps.length > 0 && (
              <div style={{ marginBottom: 20 }}>
                <div style={{ color: "#5A7186", fontSize: 12, fontFamily: "ui-monospace, monospace", textTransform: "uppercase", marginBottom: 8 }}>
                  What is missing for you
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {result.gaps.map((g, i) => (
                    <div key={i} style={{ fontSize: 14, color: "#16232E", lineHeight: 1.5, paddingLeft: 14, borderLeft: "2px solid #C0392B" }}>
                      {g}
                    </div>
                  ))}
                </div>
              </div>
            )}

            {result.competitors && result.competitors.length > 0 && (
              <div style={{ marginBottom: 20 }}>
                <div style={{ color: "#5A7186", fontSize: 12, fontFamily: "ui-monospace, monospace", textTransform: "uppercase", marginBottom: 8 }}>
                  Named instead
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 4, fontFamily: "ui-monospace, monospace", fontSize: 14 }}>
                  {result.competitors.map((c, i) => (
                    <div key={i} style={{ color: "#16232E" }}>[{i + 1}] {c}</div>
                  ))}
                </div>
              </div>
            )}

            {leadPhase === "done" ? (
              <div style={{ background: "#EAF6FC", border: "1px solid #BBDDF0", borderRadius: 8, padding: "12px 14px", fontSize: 14, color: "#16232E", marginBottom: 14 }}>
                Sent. The full report is on its way to your inbox.
              </div>
            ) : (
              <form onSubmit={submitEmail} style={{ marginBottom: 14 }}>
                <div style={{ color: "#5A7186", fontSize: 12, fontFamily: "ui-monospace, monospace", textTransform: "uppercase", marginBottom: 8 }}>
                  Want this emailed to you?
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <input
                    type="email"
                    inputMode="email"
                    autoComplete="email"
                    value={leadEmail}
                    onChange={(ev) => setLeadEmail(ev.target.value)}
                    placeholder="name@yourcompany.com"
                    style={{ ...input, flex: 1, minWidth: 0 }}
                  />
                  <button
                    type="submit"
                    disabled={leadPhase === "sending"}
                    style={{ background: leadPhase === "sending" ? "#7FB4D4" : "#0084CC", color: "#FFFFFF", border: "none", borderRadius: 8, padding: "0 18px", fontWeight: 600, fontSize: 14, cursor: "pointer", whiteSpace: "nowrap" }}
                  >
                    {leadPhase === "sending" ? "Sending" : "Send it"}
                  </button>
                </div>
                {leadErr ? (
                  <p style={{ color: "#C0392B", fontSize: 12, marginTop: 6 }}>{leadErr}</p>
                ) : null}
              </form>
            )}

            <a
              href={`/fix-plan?business=${encodeURIComponent(form.businessName)}&industry=${encodeURIComponent(form.industry)}&cited=${result.cited}&competitors=${encodeURIComponent((result.competitors || []).join(","))}&email=${encodeURIComponent(leadEmail)}`}
              style={goldBtn}
            >
              Get your fix plan <ArrowRight size={16} />
            </a>
            <p style={{ color: "#5A7186", fontSize: 12, textAlign: "center", marginTop: 12 }}>{BONUS_TEXT}</p>

            <button onClick={reset} style={{ color: "#5A7186", width: "100%", fontSize: 12, fontFamily: "ui-monospace, monospace", marginTop: 16, display: "flex", alignItems: "center", justifyContent: "center", gap: 4, background: "none", border: "none", cursor: "pointer" }}>
              <RotateCcw size={12} /> Scan another business
            </button>
          </div>
        )}

        {phase === "error" && (
          <div style={card}>
            <div style={{ color: "#C0392B", display: "flex", alignItems: "center", gap: 8, fontWeight: 600, marginBottom: 8 }}>
              <TriangleAlert size={18} /> Scan didn't run
            </div>
            <p style={{ color: "#5A7186", fontSize: 14, marginBottom: 16 }}>{errorMsg}</p>
            <button onClick={reset} style={{ background: "#D3DEE7", color: "#16232E", width: "100%", borderRadius: 8, padding: "8px 0", fontSize: 14, border: "none", cursor: "pointer" }}>
              Try again
            </button>
          </div>
        )}

        <p style={{ color: "#6E8598", fontSize: 12, textAlign: "center", marginTop: 24, lineHeight: 1.6 }}>
          No AI platform guarantees inclusion in generated answers. Results reflect a
          point-in-time check, not a permanent status.
        </p>
      </div>
    </div>
  );
}
